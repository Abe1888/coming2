...
export default function App() {
  ...
  const dismissIntroScreen = () => {
    console.log('🚀 ENTER EXPERIENCE clicked', { 
      audioSysExists: !!audioSysRef.current, 
      isMuted,
      initialized: audioSysRef.current?.initialized,
      engineStartAlreadyTriggered: engineStartTriggered.current
    });
    ...
    if (audioSysRef.current && !engineStartTriggered.current) {
      ...
      console.log('🚛 Triggering engine start (first time only)...');
      audioSysRef.current.triggerEngineStart();
      engineStartTriggered.current = true;
    } else if (engineStartTriggered.current) {
      console.log('⚠️ Engine start already triggered, skipping');
    } else {
  ...
  useEffect(() => {
    ...
    console.log('🔄 Page loaded - scroll reset to 0%');
  }, []);
  ...
  // Send heartbeat to controller (so it shows "Connected")
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      ...
      try {
        ...
        switch (e.key) {
          case 'truck-config':
            if (newValue) {
              console.log('🚛 Truck config received from controller:', newValue);
              if (truckModelRef.current) {
                // Update Three.js object DIRECTLY (no React state, no re-render!)
                ...
                truckModelRef.current.position.set(pos[0], pos[1], pos[2]);
                truckModelRef.current.rotation.set(rot[0], rot[1], rot[2]);
                truckModelRef.current.scale.set(scl[0], scl[1], scl[2]);
                console.log('✅ Truck updated in 3D scene');
              } else {
                console.warn('⚠️ Truck model ref not ready yet');
              }
            }
            break;
          case 'fuel-sensor-config':
            if (newValue) {
              console.log('⛽ Fuel sensor config received from controller:', newValue);
              if (fuelSensorGroupRef.current) {
                ...
                console.log('✅ Fuel sensor updated in 3D scene');
              } else {
                console.warn('⚠️ Fuel sensor ref not ready yet');
              }
            }
            break;
          case 'telematics-display-config':
            if (newValue) {
              console.log('📺 Telematics display config received from controller:', newValue);
              if (telematicsDisplayRef.current) {
                // Update Three.js GROUP (not mesh) DIRECTLY (no React state, no re-render!)
                const pos = newValue.position as [number, number, number];
                const rot = newValue.rotation as [number, number, number];
                const size = newValue.size as [number, number];
                
                // Update group position and rotation
                telematicsDisplayRef.current.position.set(pos[0], pos[1], pos[2]);
                telematicsDisplayRef.current.rotation.set(rot[0], rot[1], rot[2]);
                
                // Update mesh scale (first child of group)
                const mesh = telematicsDisplayRef.current.children[0] as THREE.Mesh;
                ...
                console.log('✅ Telematics display updated in 3D scene');
                console.log('  Position:', pos);
                console.log('  Rotation:', rot);
                console.log('  Size:', size);
              } else {
                console.warn('⚠️ Telematics display ref not ready yet');
              }
            }
            break;
          case 'logo-config':
            if (newValue) {
              console.log('🏷️ Logo config received from controller:', newValue);
              if (logoPlaneRef.current) {
                // Update Three.js object DIRECTLY (no React state, no re-render!)
                const pos = newValue.position as [number, number, number];
                const rot = newValue.rotation as [number, number, number];
                const scale = newValue.scale as [number, number];
                const offsetZ = newValue.offsetZ as number;
                const visible = newValue.visible as boolean;
                
                // Update position
                logoPlaneRef.current.position.set(pos[0], pos[1], pos[2]);
                
                // Update rotation
                logoPlaneRef.current.rotation.set(rot[0], rot[1], rot[2]);
                
                // Update scale
                logoPlaneRef.current.scale.set(scale[0], scale[1], 1);
                
                // Apply forward/backward offset in local space
                const forwardOffset = new THREE.Vector3(0, 0, offsetZ);
                forwardOffset.applyQuaternion(logoPlaneRef.current.quaternion);
                logoPlaneRef.current.position.add(forwardOffset);
                
                // Update visibility
                logoPlaneRef.current.visible = visible;
                
                console.log('✅ Logo updated in 3D scene');
                console.log('  📍 Position:', logoPlaneRef.current.position);
                console.log('  🔄 Rotation:', logoPlaneRef.current.rotation);
                console.log('  📏 Scale:', logoPlaneRef.current.scale);
                console.log('  👁️ Visible:', visible);
              } else {
                console.warn('⚠️ Logo plane ref not ready yet');
              }
            }
            break;
          case 'audio-state':
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (rawScroll < 0.5 && extendedModeRef.current) {
        ...
        console.log('🔄 Extended mode reset - scrolled back below 50%');
      }
      ...
      if (rawScroll >= 0.5 && !extendedModeRef.current) {
        ...
        console.log('🚀 Extended mode triggered at 50%');
      ...
      if (!airBrakeTriggered && scrollRef.current > 0) {
        if (audioSysRef.current?.masterGain && audioSysRef.current.masterGain.gain.value > 0) {
          ...
          console.log('💨 Air brake triggered at 0% scroll!');
        }
      }
      ...
      if (!hornTriggered && scrollRef.current > 0.06) {
        if (audioSysRef.current?.masterGain && audioSysRef.current.masterGain.gain.value > 0) {
          ...
          audioSysRef.current.reduceEngineVolume(); // Reduce engine to 15% background level
          hornTriggered = true;
          console.log('🎺 Horn triggered at 6% scroll!');
        }
      }
    };
  ...
  return (
    <div className="app-container">
      ...
      {showIntroScreen && (
        ...
        >
          <div className="intro-content">
            <img src="/logo.png" alt="Translink" className="intro-logo" />
      ...
      {sceneRef.current && truckRef.current && lightGrayMatRef.current && edgeMatRef.current && (
        <TruckModel
          ...
          onLoad={(model, wheels) => {
            ...
            truckModelRef.current = model; // Store ref for controller updates
            
            // Apply initial config from localStorage
            const saved = localStorage.getItem('truck-config');
            if (saved) {
              ...
              console.log('✅ Truck initial config loaded from localStorage');
            }
            
            setLoadingProgress(100);
            // Model loaded - show enter button (don't auto-dismiss)
          }}
          onLogoCreated={(logoPlane) => {
            logoPlaneRef.current = logoPlane;
            
            // Apply initial logo config from localStorage
            const saved = localStorage.getItem('logo-config');
            if (saved) {
              const config = JSON.parse(saved);
              logoPlane.position.set(config.position[0], config.position[1], config.position[2]);
              logoPlane.rotation.set(config.rotation[0], config.rotation[1], config.rotation[2]);
              logoPlane.scale.set(config.scale[0], config.scale[1], 1);
              
              // Apply forward/backward offset
              const forwardOffset = new THREE.Vector3(0, 0, config.offsetZ);
              forwardOffset.applyQuaternion(logoPlane.quaternion);
              logoPlane.position.add(forwardOffset);
              
              logoPlane.visible = config.visible;
              console.log('✅ Logo initial config loaded from localStorage');
            }
          }}
        />
      )}
      ...
      {sceneRef.current && truckRef.current && (
        <TelematicsDisplay
          scene={sceneRef.current}
          truckGroup={truckRef.current}
          ...
          onGroupCreated={(group) => {
            ...
            console.log('✅ Telematics display group initialized from objectTransforms.json');
          }}
        />
      )}
      ...
      {sceneRef.current && truckRef.current && (
        <FuelSensor
          ...
          onComponentsCreated={(components) => {
            ...
            // Store the parent group for direct updates
            if (components.sensorHeadGroup.parent) {
              fuelSensorGroupRef.current = components.sensorHeadGroup.parent as THREE.Group;
              
              // Apply initial config from localStorage
              const saved = localStorage.getItem('fuel-sensor-config');
              if (saved) {
                const config = JSON.parse(saved);
                fuelSensorGroupRef.current.position.set(config.position[0], config.position[1], config.position[2]);
                fuelSensorGroupRef.current.rotation.set(config.rotation[0], config.rotation[1], config.rotation[2]);
                fuelSensorGroupRef.current.scale.setScalar(config.scale);
                console.log('✅ Fuel sensor initial config loaded from localStorage');
              }
            }
          }}
        />
